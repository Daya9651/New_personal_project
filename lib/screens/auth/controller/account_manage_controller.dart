  ...
  Future confirmUserApi({String confirm = "no"}) async {
    ...
    try {
      ...
      dio.Response response = await ApiService.postData(
          url: confirmationLawyerAndClientUrl, data: data);
...
// todo state api
  ...
  Future stateGet() async {
    cityData.value = [];
    try {
      dio.Response response = await ApiService.getData(stateUrl,
          queryParameters: {'country_id': 1});
  ...
  Future cityGet(stateID) async {
    ...
    try {
      ...
      dio.Response response = await ApiService.getData(cityUrl,
          queryParameters: {'state_id': stateID});
      var data = response.data['data'] as List;
      cityData.value =
          data.map((item) => CityData.fromJson(item)).toList().toSet().toList();
